...
echo $_SESSION['name'];
	...
	$sql = "SELECT open FROM admin_schedule WHERE exam = 'Practical Exam'";
...
<body>
				...
				<div class="card-box table-responsive">
					<table id="datatable-responsive" class="table table-bordered table-striped dt-responsive nowrap" cellspacing="0" width="100%">
						...
						<tbody>
							<?php 
								require_once 'connection.php'; 
								$sql = "SELECT * FROM client_schedule WHERE typeofexam = 'Practical Exam'";
								$query = $conn->query($sql);
								while($row = $query->fetch_assoc()){						
							?>
				...
				<div class="card-box table-responsive">
					<table id="datatable-responsive" class="table table-bordered table-striped dt-responsive nowrap" cellspacing="0" width="100%">
						...
						<tbody>
							<?php 
								require_once 'connection.php'; 
								$sql = "SELECT * FROM client_scheduleonly WHERE typeofexam = 'Practical Exam'";
								$query = $conn->query($sql);
								while($row = $query->fetch_assoc()){						
							?>
	...
	<div>
		<center>
			<a class="btn btn-info" href="practical.php">Back to Scheduling</a>
		</center>
	</div>
	...
	<div id="remove" class="modal fade" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-footer">
				<center>
					<form action="schedule_removeP.php" method="POST">
	...
	<div id="submit" class="modal fade" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-body">
				<form action="submit_adminP.php" method="POST" enctype="multipart/form-data">	
							<center>
							<?php 
								require_once 'connection.php'; 
								$sql = "SELECT * FROM client_schedule WHERE typeofexam = 'Practical Exam'";
								$query = $conn->query($sql);
								while($row = $query->fetch_assoc()){	
									$isql = "SELECT price FROM vehicles WHERE category = '" . $row['vehicle_type'] . "'";
							...
							?>
								<p style="font-weight:bold; color:red;">Note: Please pay first before getting your schedule</p>
								<p style="font-weight:bold;" class="alert-success">Total Cost: <?php echo $price; ?></p>
								<p><b>Company G-Cash Number: 09xxxxxxxxx</b></p>
								<p><b>Please Attach Your Proof Of Payment Below</b></p>
								<div class="image-container" style="width:100%; height:auto;">
									<input type="file" name="image" accept="image/*" onchange="previewImage(event)">
									<img id="image-preview" src="#" alt="Image Preview" style="max-width:100%; max-height:100%;">
								</div>
								<input type="hidden" name="student_id" value="<?php echo $_SESSION['student_id']; ?>">
								<input type="hidden" name="name" value="<?php echo $_SESSION['name']; ?>">
								<input type="number" name="payment" placeholder="Payment" style="margin-top:20px;" required>
							<?php }  ?>
							</center>
						</div>
						<div class="modal-footer">
							<center>
								...
								<?php }?>
							<button class="btn btn-default" name="submit">Submit Schedule</button>
